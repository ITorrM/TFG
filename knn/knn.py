...
def train_eval_model():
    ...
    sns.heatmap(cm.astype('float') / cm.sum(axis=1)[:, np.newaxis], annot=True, fmt='.2%', cmap='Oranges',
                xticklabels=class_names, yticklabels=class_names)
    plt.ylabel('Etiqueta real', fontsize=12)
    plt.xlabel('Etiqueta estimada', fontsize=12)
    plt.tight_layout()
    plt.savefig('resultados/matriz_confusion_knn.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('resultados/curvas_pr_knn.png', dpi=300)
    plt.close()
