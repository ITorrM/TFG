...
def train_eval_model():
    ...
    plt.tight_layout()
    plt.savefig('resultados/matriz_confusion_svm.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('resultados/curvas_pr_svm.png', dpi=300)
    plt.close()

    
    svm_model = svm_pipeline.named_steps['svm']
    scaler_model = svm_pipeline.named_steps['scaler']
    ...
    explainer = shap.LinearExplainer(svm_model, X_train_transformed)
    ...
    plt.tight_layout()
    plt.savefig('resultados/shap_resumen_ataques_svm.png', dpi=300, bbox_inches='tight')
    plt.close() 
